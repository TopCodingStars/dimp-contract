...
contract PassportMint is ERC721Enumerable, Ownable {
    ...
    function mint(
        ...
        string memory ownReferralLink,
        string memory email
    ) external {
        require(
            _nftIdByAddr[msg.sender] == 0 && _nftIdByEmail[email] == 0,
            "Your account has already used for passport!"
        );
        require(_nftIdByReferral[ownReferralLink] == 0, "Invalid Referral Link");
        require(
            _nftIdByReferral[referer] > 0 ||
                keccak256(abi.encodePacked(referer)) == keccak256(abi.encodePacked("no")),
            "Your referer is invalid"
        );
        pos = pos + 1;
        uint256 levelTemp;
        if (keccak256(abi.encodePacked(referer)) == keccak256(abi.encodePacked("no"))) {
            ...
            _totalInfo[_nftIdByReferral[referer]].referralCount =
                _totalInfo[_nftIdByReferral[referer]].referralCount +
                1;
            if (
                keccak256(abi.encodePacked(_totalInfo[_nftIdByReferral[referer]].referer)) ==
